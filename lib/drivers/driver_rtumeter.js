...
class RTUMeter extends DriverBase {
	constructor(stateInstance, inverter, options) {
		...
		this._rtumeterRegMap={
			...
			2126: { id: 'meter.activePower', conv: -0.001,liveval: true },
	...
	initSerial() {
		...
		this.serialport = new SerialPort({ path: this.adapter.settings.rtumeter.device, baudRate: 9600 });
		const { InterByteTimeoutParser } = require('@serialport/parser-inter-byte-timeout');
		this.parser = this.serialport.pipe(new InterByteTimeoutParser({ interval: 50 }));
		this.ts = 0;
		this.serialport.on("open", () => { this.log.debug("rtumeter.port opened") });
	...
	async ondata(data) {
		...
		for(let i=0;i+4+offset<data.length;i+=4){
			...
			if (register){
				...
				this.stateCache.set(register.id,value,{type: 'number', stored: register.liveval? register.liveval : false})
				if (register.liveval){
